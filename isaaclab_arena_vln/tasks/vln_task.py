...
from isaaclab_arena_vln.metrics.vln_metrics import (
...
def vln_stop_term(env) -> torch.Tensor:
    ...
    return torch.as_tensor(flag, device=env.device, dtype=torch.bool)
...
def vln_stuck_term(env, velocity_threshold: float = 0.1, patience: int = 1000) -> torch.Tensor:
    ...
    if stuck_mask.any():
        ids = stuck_mask.nonzero(as_tuple=False).flatten().tolist()
        print(f"[VlnNavTask] Robot stuck for {patience} steps in env(s) {ids}")
    return stuck_mask
...
def reset_robot_and_goal_from_vln_dataset(
    ...
    # Teleport all reset robots in one batched call
...
class VlnNavTask(TaskBase):
    """Vision-Language Navigation task that loads R2R episodes.

    This task:
      - Reads episodes from a gzipped JSON dataset.
      - Provides termination (time-out + goal-reached).
      - Registers VLN metrics (SPL, Success, PathLength, DistanceToGoal).
      - On reset, teleports the robot to the episode start pose and stores
        the instruction / goal in ``env.extras``.

    Multi-env support:
      Works with ``num_envs >= 1``.  Each env gets its own episode.
      Note that Matterport scenes are large — running many envs in parallel
      may require significant GPU memory since each env loads a full copy
      of the scene USD.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            success_radius: Distance threshold (meters) for goal success.
        """
        super().__init__(
        ...
        if episode_indices is not None:
            raw_episodes = [raw_episodes[i] for i in episode_indices]

        vln_episodes = [build_vln_episode_from_raw(ep) for ep in raw_episodes]
        self.vln_benchmark_cfg = VlnBenchmarkCfg(episodes=vln_episodes)
        ...
        self._success_radius_per_episode: list[float] = [success_radius] * len(vln_episodes)
    ...
    def get_termination_cfg(self) -> VlnTerminationsCfg:
        return VlnTerminationsCfg(
            success=TerminationTermCfg(
                func=vln_success_term,
                params={"position_tolerance": 0.3},
            ),
    ...
    def get_metrics(self) -> list[MetricBase]:
        return [
            ...
            SuccessMetric(self._gt_waypoints_per_episode, self._success_radius_per_episode),
            SPLMetric(self._gt_waypoints_per_episode, self._success_radius_per_episode),
        ]
