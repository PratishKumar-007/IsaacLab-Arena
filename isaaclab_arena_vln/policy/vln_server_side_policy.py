...
"""VLN server-side policy: wraps a VLM model for navigation command generation.
...
Launch via the standard Arena remote policy server runner::
    ...
    python -m isaaclab_arena.remote_policy.remote_policy_server_runner \\
        --policy_type isaaclab_arena_vln.policy.vln_server_side_policy.VlnServerSidePolicy \\
...
@dataclass
class VlnServerSidePolicyConfig:
    """Configuration for :class:`VlnServerSidePolicy`."""
...
class VlnServerSidePolicy(ServerSidePolicy):
    """Server-side VLN policy backed by a NaVILA / LLaVA VLM."""

    config_class = VlnServerSidePolicyConfig

    def __init__(self, config: VlnServerSidePolicyConfig) -> None:
        ...
        print(f"[VlnServerSidePolicy] Loading VLM model from: {self._model_path}")
        ...
        print("[VlnServerSidePolicy] VLM model loaded successfully.")
    ...
    def _build_protocol(self) -> ActionProtocol:
        return VlnVelocityActionProtocol(
            action_dim=3,
            observation_keys=["camera_obs.robot_head_cam.rgb"],
    ...
    def set_task_description(self, task_description: str | None) -> dict[str, Any]:
        ...
        print(f"[VlnServerSidePolicy] Instruction: {self._instruction[:80]}...")
    ...
    def get_action(
        ...
        if rgb_np is not None:
            ...
            if len(self._image_history) > self._num_video_frames:
                self._image_history = self._image_history[-self._num_video_frames:]
    ...
    def _run_vlm_inference(self) -> str:
        ...
        if len(self._image_history) < self._num_video_frames:
            frames = [self._image_history[0]] * (self._num_video_frames - len(self._image_history))
            frames += self._image_history
        else:
            frames = self._image_history[-self._num_video_frames:]
    ...
    @staticmethod
    def from_args(args: argparse.Namespace) -> VlnServerSidePolicy:
        config = VlnServerSidePolicyConfig(
        ...
        return VlnServerSidePolicy(config)
