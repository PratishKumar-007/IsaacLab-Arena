...
class RobotPositionRecorder(RecorderTerm):
    ...
    def record_post_step(self):
        ...
        robot_pos = self._env.scene["robot"].data.root_pos_w.clone().cpu().numpy()
        return self.name, robot_pos
...
class DistanceToGoalMetric(MetricBase):
    ...
    def compute_metric_from_recording(self, recorded_metric_data: List[np.ndarray]) -> float:
        if not recorded_metric_data:
            return 0.0
        assert len(recorded_metric_data) == len(self._gt_waypoints)

        distances: List[float] = []
        for ep_idx, ep_data in enumerate(recorded_metric_data):
            pos = _episode_pos_array(ep_data)
            if pos.shape[0] == 0:
            ...
            tree = KDTree(gt_wps)
            closest_dist, closest_idx = tree.query(final_pos)
            total = float(closest_dist)
            for i in range(closest_idx, len(gt_wps) - 1):
                total += float(np.linalg.norm(gt_wps[i + 1] - gt_wps[i]))
...
class SuccessMetric(MetricBase):
    ...
    def compute_metric_from_recording(self, recorded_metric_data: List[np.ndarray]) -> float:
        if not recorded_metric_data:
            return 0.0
        assert len(recorded_metric_data) == len(self._radii)
...
class SPLMetric(MetricBase):
    ...
    def compute_metric_from_recording(self, recorded_metric_data: List[np.ndarray]) -> float:
        if not recorded_metric_data:
            return 0.0
        assert len(recorded_metric_data) == len(self._shortest)

        spl_values: List[float] = []
        for i, ep_data in enumerate(recorded_metric_data):
            pos = _episode_pos_array(ep_data)

            # Actual path length l_i
            if pos.shape[0] < 2:
                l_i = 0.0
            else:
                l_i = float(np.linalg.norm(pos[1:] - pos[:-1], axis=-1).sum())
